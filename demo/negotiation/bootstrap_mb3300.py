"""MB-3300 — Competing Customers: World Bootstrap.

Real-API-only, same principle as every other demo bootstrap this
session. Two Customer actors, Alice and Bob, each with a real strategy
profile (POST /actors body.metadata["strategy"]) — different real
preference weights, not identical actors racing for identical reasons.
One product with quantity=1 — genuinely only one can win.
"""
...
def build_society(c: httpx.Client, spaces: dict[str, str]) -> dict[str, str]:
    result = call(
        ...
        json={
            ...
            "description": "Customers competing for real stock",
        },
    )
...
# Different real preference weights — Alice values speed, Bob values
# cost — so a genuine competition has two actors with different real
# reasons for wanting the same thing, not two identical clones.
ACTOR_DEFS = (
    ("Alice", {"speed": 0.9, "cost": 0.1}, "competitive"),
    ("Bob", {"cost": 0.8, "speed": 0.2}, "competitive"),
)

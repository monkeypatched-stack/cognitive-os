"""MB-3102 — Payment Declined: World Bootstrap.

Same real-API-only principle as bootstrap_mb3100.py. Alice has no
wallet, so her Payment step fails deterministically (observed in every
run this session: "no wallet account found for payment") — a reliable,
real PaymentDeclined trigger, not a contrived one.

Inventory Society subscribes to "PaymentDeclined". Logistics subscribes
to "InventoryReserved" and should stay idle (nothing in this scenario
ever publishes it). A reservation is pre-seeded via the real
POST /inventory/reserve API (hold_seconds=600, well past the whole
scenario's real runtime) — real production API, not a KG hack — so
...
ACTOR_DEFS = (
    ...
    # A neutral goal, not "reserve_inventory" — found live: a reactive
    # (untargeted) tick currently surfaces zero observed facts about
    # WHY the actor woke up (SocietyRuntime.tick()'s untargeted mode
    # has no way to broadcast a reason to members), so a goal that
    # already says "reserve" reliably biased the planner toward
    # InventoryReserve regardless of the real trigger (PaymentDeclined,
    # which needs InventoryRelease instead).

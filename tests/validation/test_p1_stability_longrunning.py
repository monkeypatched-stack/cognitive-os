...
class ResourceMonitor:
    ...
    def get_metrics(self) -> StabilityMetrics:
        ...
        with self.lock:
            ...
            orphaned_leases_peak = max((s.active_leases for s in self.snapshots), default=0)
...
class SimulatedActor:
    ...
    def start(self, tick_interval: float = 0.1):
        ...
        self.should_stop = False
        self.thread = threading.Thread(target=self.run_tick_loop, args=(tick_interval,), daemon=True)
        self.thread.start()
...
class TestStability24Hour:
    ...
    def test_4_actors_stable_over_simulated_24_hours(self):
        ...
        # Create 4 actors
        actors = [SimulatedActor(f"actor_{i}", monitor) for i in range(4)]
        ...
        assert memory_growth < 10, f"Memory leak detected: {memory_growth:.2f} MB/hour growth rate"

        # 2. No orphaned leases
        assert metrics.orphaned_leases_peak == 0, f"Orphaned leases detected: peak {metrics.orphaned_leases_peak}"

        # 3. Audit log grows
        assert metrics.audit_log_growth_rate > 0, "Audit log not growing (no ticks executed?)"
        ...
        print(
            f"   Memory: {metrics.snapshots[0].process_memory_mb:.1f}MB → {metrics.snapshots[-1].process_memory_mb:.1f}MB"
        )

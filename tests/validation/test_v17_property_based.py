    ...
    def test_1000_random_child_delegations_never_exceed_their_parents_bounds(self):
        ...
        for i in range(1000):
            ...
            parent = issue_delegation(
                ...
                constraints={"max_amount": parent_amount, "region": "IN"},
            ...
            child = issue_delegation(
                ...
                capabilities=("grocery.purchase",),
                scope=parent.scope,
                constraints={"max_amount": child_amount, "region": "IN"},
            ...
            if child.constraints["max_amount"] > parent.constraints["max_amount"]:
                violations.append(
                    (
                        "amount",
                        i,
                        child.constraints["max_amount"],
                        parent.constraints["max_amount"],
                    )
                )
            if child.expires_at > parent.expires_at:
                violations.append(("expiry", i, child.expires_at, parent.expires_at))
        assert violations == [], (
            f"delegation attenuation violated for {len(violations)}/1000 random cases: {violations[:5]}"
        )
        ...
        for i in range(1000):
            ...
            try:
                issue_delegation(
                    ...
                    capabilities=("grocery.purchase",),
                    scope=parent.scope,
                    constraints={"max_amount": widened_amount, "region": "IN"},
                    ttl_seconds=1800,
                    parent=parent,
                )
                ...
                pass
        assert not_rejected == [], (
            f"{len(not_rejected)}/1000 over-widened delegations were NOT rejected: {not_rejected[:5]}"
        )
    ...
    def test_100_random_capability_resource_pairs_all_require_real_authorization(self):
        ...
        async def _deny_everything(action, resource, extra, *, verified_delegation=None):
            return {
                "allowed": False,
                "approval_mode": "DENY",
                "reason": "property-test fail-closed default",
            }
        ...
        assert bypasses == [], (
            f"found {len(bypasses)} governance bypasses among 100 random capability/resource pairs: {bypasses[:5]}"
        )

...
class TestAgentSandboxCapabilityGating:
    ...
    def test_sandbox_capability_gating(self):
        ...
        limits = SandboxLimits(allowed_capabilities={"read", "write"}, denied_capabilities=set())
        sandbox = AgentSandbox(limits)
        ...
        denied_limits = SandboxLimits(denied_capabilities={"network"})
...
class TestSandboxIntegrationWithActorRuntime:
    ...
    def test_sandbox_creation_factory_by_agent_type(self):
        ...
        """
        from src.monkey_brain.kernel.execute.sandbox import (
            create_sandbox,
            SandboxLimits,
        )
        ...
        for agent_type in types:
            ...
            print(
                f"✅ {agent_type}: timeout={limits.timeout_seconds}s, "
    ...
    def test_untrusted_agent_has_strictest_limits(self):
        ...
        print(f"   Untrusted: {untrusted._limits.max_steps} steps, {untrusted._limits.timeout_seconds}s timeout")
        print(f"   Government: {government._limits.max_steps} steps, {government._limits.timeout_seconds}s timeout")
...
class TestSandboxSecurityBoundaries:
    ...
    def test_sandbox_prevents_capability_exploitation(self):
        ...
        limits = SandboxLimits(allowed_capabilities={"compute"}, denied_capabilities=set())
        sandbox = AgentSandbox(limits)

...
PERFORMANCE_BUDGETS = {
    ...
    "planetary.cycle_per_actor": LatencyBudget(
        name="planetary.cycle_per_actor",
        target_ms=2500,
        p50_ms=3000,
        p95_ms=4500,
        p99_ms=6000,
...
class MemoryBudget:
    ...
    def check(self, growth_kb: float, units: int) -> tuple[bool, str]:
        ...
        if growth_kb > budget_kb:
            return (
                False,
                f"exceeded budget ({growth_kb:.0f}KB > {budget_kb:.0f}KB for {units} units)",
            )
        return True, "within budget"
...
class PerformanceMonitor:
    ...
    def get_stats(self, operation: str) -> dict[str, Any]:
        ...
        return {
            ...
            "max_ms": obs_sorted[-1],
            "budget": (PERFORMANCE_BUDGETS.get(operation, {}).to_dict() if operation in PERFORMANCE_BUDGETS else None),
        }

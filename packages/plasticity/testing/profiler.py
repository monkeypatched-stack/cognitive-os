...
class Profiler:
    ...
    def cpu_profile(self, name: str, **metadata: Any):
        """Profile CPU usage."""
        entry = ProfileEntry(
            name=name, category="cpu", start_time=time.monotonic(), metadata=metadata
        )
        try:
            yield entry
        finally:
    ...
    def memory_profile(self, name: str, **metadata: Any):
        ...
        entry = ProfileEntry(
            name=name, category="memory", start_time=time.monotonic(), metadata=metadata
        )
        try:
            yield entry
        finally:
    ...
    def io_profile(self, name: str, **metadata: Any):
        ...
        entry = ProfileEntry(
            name=name, category="io", start_time=time.monotonic(), metadata=metadata
        )
        try:
            yield entry
        finally:
    ...
    def set_budget(
        self, name: str, max_ms: float = 1000.0, warning_threshold: float = 0.8
    ) -> None:

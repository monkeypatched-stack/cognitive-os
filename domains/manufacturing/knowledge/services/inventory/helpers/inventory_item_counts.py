...
COLLECTION = "inventory_item_counts"
...
def _as_response_doc(data: dict) -> dict:
    return _prepare(InventoryItemCountResponse(**data).model_dump())
...
async def create(db: AsyncIOMotorDatabase, data: InventoryItemCountCreate) -> dict:
    now = utc_now()

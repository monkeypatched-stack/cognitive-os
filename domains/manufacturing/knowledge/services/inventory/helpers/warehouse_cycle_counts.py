...
COLLECTION = "warehouse_cycle_counts"
...
def _as_response_doc(data: dict) -> dict:
    return _prepare(WarehouseCycleCountResponse(**data).model_dump())
...
async def create(db: AsyncIOMotorDatabase, data: WarehouseCycleCountCreate) -> dict:
    now = utc_now()
    cycle_count_id = f"warehouse-cycle-count-{uuid4().hex[:12]}"
    base = data.model_dump()
    if not base.get("cycle_count_number"):
        base["cycle_count_number"] = f"CC-{uuid4().hex[:8].upper()}"
    doc = _prepare(
        {
            **base,

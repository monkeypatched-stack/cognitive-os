...
from services.customers.helpers import customer_order_metrics as crud
from services.common.auth import require_permission
from services.customers.models.customer_order_metrics import (
...
async def list_customer_order_metrics(
...
@router.get("/{customer_id}", response_model=CustomerOrderMetricsResponse)
async def get_customer_order_metrics(
    customer_id: str,
    db: AsyncIOMotorDatabase = Depends(get_database),
    _: dict = Depends(require_permission("perm-view-products")),
):
    ...
    if not record:
        raise HTTPException(
            ...
            detail=f"Customer order metrics for customer '{customer_id}' not found",
        )
    return record
...
async def create_customer_order_metrics(
    ...
    if data.customer_id and await crud.get_by_customer_id(db, data.customer_id):
        raise HTTPException(
            ...
            detail=f"Customer order metrics for customer '{data.customer_id}' already exist",
        )
    return await crud.create(db, data)


@router.patch("/{customer_id}", response_model=CustomerOrderMetricsResponse)
async def update_customer_order_metrics(
    customer_id: str,
...
async def delete_customer_order_metrics(
    customer_id: str,
    db: AsyncIOMotorDatabase = Depends(get_database),

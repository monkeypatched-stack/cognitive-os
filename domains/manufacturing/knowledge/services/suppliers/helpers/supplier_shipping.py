...
async def get_by_id(
    db: AsyncIOMotorDatabase, supplier_shipping_id: str
) -> Optional[dict]:
    return _serialize(
        await db[COLLECTION].find_one({"supplier_shipping_id": supplier_shipping_id})
    )
...
async def create(db: AsyncIOMotorDatabase, data: SupplierShippingCreate) -> dict:
    doc = {
        **data.model_dump(),
        "supplier_shipping_id": f"supplier-ship-{uuid4().hex[:12]}",
    }
...
async def update(
    db: AsyncIOMotorDatabase, supplier_shipping_id: str, data: SupplierShippingUpdate
) -> Optional[dict]:
    fields = data.model_dump(exclude_unset=True)
    if not fields:
        return await get_by_id(db, supplier_shipping_id)
    result = await db[COLLECTION].find_one_and_update(
        {"supplier_shipping_id": supplier_shipping_id},
        {"$set": fields},
...
async def delete(db: AsyncIOMotorDatabase, supplier_shipping_id: str) -> bool:
